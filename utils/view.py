...
def main():
    ...
    viewer = ImageViewerWidget(options)
    viewer.setNumStreams(1)

    window = QtWidgets.QMainWindow()
    window.setCentralWidget(viewer)
    window.setFocusPolicy(QtCore.Qt.FocusPolicy.StrongFocus)
    window.setWindowTitle(f'View {filename}')
    window.resize(1600, 1000)
    window.show()

    viewer.set_frame(0, w, h, format, buf)
